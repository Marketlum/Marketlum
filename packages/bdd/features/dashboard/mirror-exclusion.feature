Feature: Dashboard mirror exclusion
  ...
  dashboard aggregates, so a deal invoiced on behalf of a sub-actor is
  counted exactly once. Genuine internal invoices remain included.

  Background:
    Given I am authenticated as "admin@marketlum.com"
    And a value exists with name "USD"
    And an actor exists named "Holding" of type "organization"
    And an actor exists named "Studio" of type "virtual" under parent "Holding"
    And an actor exists named "Customer" of type "organization"
