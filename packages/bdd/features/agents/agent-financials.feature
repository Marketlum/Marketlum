Feature: Agent financials
  ...
  revenue, invoices the agent received are expense — the invoice direction
  column encodes the value-stream perspective and is ignored here. Figures
  ...
  Scenario: Issued invoices are revenue and received invoices are expense
    Given an invoice exists from "Acme" to "Globex" issued "2026-01-15" amount "1000" with direction "revenue"
    And an invoice exists from "Globex" to "Acme" issued "2026-02-10" amount "400" with direction "expense"
  ...
  Scenario: The invoice direction column does not affect the agent P&L
    Given an invoice exists from "Acme" to "Globex" issued "2026-01-15" amount "500" with direction "expense"
    When I request the financials of "Acme" for year 2026
    Then the response status should be 200
    And the agent financials annual revenue should be "500.00"
    And the agent financials annual expense should be "0.00"

  Scenario: Figures land in the correct month and quarter
    Given an invoice exists from "Acme" to "Globex" issued "2026-03-10" amount "1200" with direction "revenue"
    When I request the financials of "Acme" for year 2026
    Then the response status should be 200
  ...
  Scenario: Amounts are converted into the agent's functional currency
    ...
    And an invoice exists from "Euro Corp" to "Globex" in "USD" issued "2026-01-15" amount "100" with direction "revenue"
  ...
  Scenario: Invoices without a per-agent snapshot are excluded and counted
    ...
    And an agent exists named "Euro Corp" with functional currency "EUR"
    And an invoice exists from "Euro Corp" to "Globex" in "USD" issued "2026-01-15" amount "100" with direction "revenue"
    And an invoice exists from "Euro Corp" to "Globex" in "GBP" issued "2026-02-15" amount "300" with direction "revenue"
    When I request the financials of "Euro Corp" for year 2026
    Then the response status should be 200
    And the agent financials annual revenue should be "200.00"
    And the agent financials invoiceCount should be 2
  ...
  Scenario: Invoices outside the requested year are excluded
    Given an invoice exists from "Acme" to "Globex" issued "2025-12-31" amount "999" with direction "revenue"
    And an invoice exists from "Acme" to "Globex" issued "2026-01-01" amount "100" with direction "revenue"
    When I request the financials of "Acme" for year 2026
    Then the response status should be 200
  ...
  Scenario: Unpaid invoices are included in the accrual view
    Given an unpaid invoice exists from "Acme" to "Globex" issued "2026-01-15" amount "750" with direction "revenue"
    When I request the financials of "Acme" for year 2026
    Then the response status should be 200
  ...
  Scenario: An agent without a functional currency gets null figures with counts
    ...
    And an invoice exists from "NoCur" to "Globex" issued "2026-01-15" amount "100" with direction "revenue"
  ...
  Scenario: A self-invoice counts as both revenue and expense
    Given an invoice exists from "Acme" to "Acme" issued "2026-01-15" amount "100" with direction "revenue"
    When I request the financials of "Acme" for year 2026
    Then the response status should be 200
    And the agent financials annual revenue should be "100.00"
