Feature: Consolidated agent financials

  With consolidated=true the agent financials cover the agent's whole
  ...
  Amounts stay in the consolidating agent's functional currency; a subtree
  invoice whose own side-agent uses a different functional currency cannot
  be summed and is surfaced through notConvertedCount instead.

  Background:
    Given I am authenticated as "admin@marketlum.com"
    And a currency value exists named "USD"
    And an agent exists named "Holding" of type "organization" with functional currency "USD"
    And an agent exists named "Subsidiary" of type "organization" with functional currency "USD" under parent "Holding"
    And an agent exists named "Studio" of type "virtual" with functional currency "USD" under parent "Holding"
    And an agent exists named "Customer" of type "organization" with functional currency "USD"
  ...
  Scenario: Standalone view does not include descendant revenue
    ...
    Then the response status should be 200
    And the agent financials annual revenue should be "0.00"
    And the agent financials invoiceCount should be 0
  ...
  Scenario: Intercompany internal invoices are eliminated in the consolidated view
    ...
    And the agent financials annual net should be "100.00"
    And the agent financials invoiceCount should be 1
  ...
  Scenario: A descendant with a different functional currency is counted as not converted
    ...
    And an agent exists named "Euro Sub" of type "organization" with functional currency "EUR" under parent "Holding"
  ...
  Scenario: Consolidated financials of a leaf agent equal the standalone view
    ...
    Then the response status should be 200
    And the agent financials annual revenue should be "300.00"
    And the agent financials invoiceCount should be 1
