Feature: Search Invoices
  ...
  Scenario: Filter by fromActorId
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Seller A"
    And an actor exists with name "Seller B"
    And an actor exists with name "Buyer Inc"
    And a value exists with name "USD"
    ...
    When I search invoices with fromActorId for "Seller A"
    Then the response status should be 200
    And the total count should be 1

  Scenario: Filter by toActorId
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Seller Corp"
    And an actor exists with name "Buyer A"
    And an actor exists with name "Buyer B"
    And a value exists with name "USD"
    ...
    When I search invoices with toActorId for "Buyer A"
    Then the response status should be 200
    And the total count should be 1

  Scenario: Filter by actorId matching either side
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Acme Corp"
    And an actor exists with name "Seller Corp"
    And an actor exists with name "Buyer Inc"
    And a value exists with name "USD"
    And an invoice exists with number "INV-001" from "Acme Corp" to "Buyer Inc"
    And an invoice exists with number "INV-002" from "Seller Corp" to "Acme Corp"
    And an invoice exists with number "INV-003" from "Seller Corp" to "Buyer Inc"
    When I search invoices with actorId for "Acme Corp"
    Then the response status should be 200
    And the total count should be 2
