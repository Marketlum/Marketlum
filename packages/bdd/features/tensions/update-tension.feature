Feature: Update Tension
  ...
  Scenario: Update all fields
    ...
    And an actor exists with name "Original Actor"
    And an actor exists with name "New Actor"
    ...
    And the response should contain an actor with name "New Actor"
    And the response should contain a lead with name "New Lead"

  Scenario: Partial update only name
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Partial Actor"
  ...
  Scenario: Update with invalid score fails
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Score Actor"
    And a tension exists with name "Score Tension"
    When I update the tension "Score Tension" with score 15
    Then the response status should be 400

  Scenario: Update with non-existent actorId fails
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Exist Actor"
    And a tension exists with name "Actor Tension"
    When I update the tension "Actor Tension" with non-existent actorId
    Then the response status should be 404

  Scenario: Update with non-existent leadUserId fails
    Given I am authenticated as "admin@marketlum.com"
    And an actor exists with name "Lead Actor"
