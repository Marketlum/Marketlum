Feature: RDHY platforms
  ...
  The RenDanHeYi plugin groups core actors into platforms. Platforms are
  ...
  Scenario: Listing platforms ordered by name with member counts
    ...
    And the actor "Washing Machines Co" is assigned to the RDHY platform "industrial_platform"
  ...
  Scenario: Platform detail includes member actor summaries
    Given an RDHY platform exists with code "industrial_platform" and name "Industrial Platform"
    And an actor exists with name "Washing Machines Co"
    And the actor "Washing Machines Co" is assigned to the RDHY platform "industrial_platform"
    When I get the RDHY platform "industrial_platform"
    Then the response status should be 200
    And the RDHY platform detail lists the member actor "Washing Machines Co"

  Scenario: Updating a platform's name and description
    Given an RDHY platform exists with code "industrial_platform" and name "Industrial Platform"
    When I update the RDHY platform "industrial_platform" with name "Industry Platform" and description "Hosts industrial actors"
    Then the response status should be 200
  ...
  Scenario: Deleting a platform detaches its member actors
    Given an RDHY platform exists with code "industrial_platform" and name "Industrial Platform"
    And an actor exists with name "Washing Machines Co"
    And the actor "Washing Machines Co" is assigned to the RDHY platform "industrial_platform"
