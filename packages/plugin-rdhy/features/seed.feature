Feature: RDHY sample seeding
  ...
  existing actors to them. Running it again changes nothing.

  Background:
    Given I am authenticated as "admin@marketlum.com"

  Scenario: The seed hook creates sample platforms and assigns actors idempotently
    Given an actor exists with name "Home Appliances Co"
    And an actor exists with name "Smart Devices Co"
    ...
    And the actor "Home Appliances Co" is assigned to an RDHY platform
    And the actor "Smart Devices Co" is assigned to an RDHY platform
  ...
  Scenario: The seed hook creates the sample EMC agreement idempotently
    Given an actor exists with name "Home Appliances Co"
    And an actor exists with name "Smart Devices Co"
    And an actor exists with name "Web3 Consulting Co"
    When the RDHY plugin seed hook runs
    And the RDHY plugin seed hook runs again
