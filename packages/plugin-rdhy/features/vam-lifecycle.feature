Feature: RDHY VAM agreement lifecycle
  ...
  while another ACTIVE agreement exists for the same value stream.
  ...
  VAM agreement; deleting the anchored value stream through core cascades
  the agreement away.
  ...
  Scenario: Only one active agreement per value stream
    Given the VAM agreement "Web 3 Consulting HUB" is activated
    And a VAM agreement titled "Parallel Cycle" exists for the value stream "web3_stream" sponsored by "industrial_platform"
  ...
  Scenario: Terminating citing another agreement's rule fails
    Given the canvas of the VAM agreement "Web 3 Consulting HUB" is replaced with the sample canvas
    And the VAM agreement "Web 3 Consulting HUB" is activated
    And a VAM agreement titled "Other Agreement" exists for the value stream "web3_stream" sponsored by "industrial_platform"
  ...
  Scenario: Platform deletion is blocked while sponsoring agreements
    ...
    Then the response status should be 409
    When I delete the value stream "web3_stream" through the core API
