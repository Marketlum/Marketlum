Feature: RDHY VAM agreement lifecycle
  ...
  while another ACTIVE agreement exists for the same actor.
  ...
  VAM agreement; deleting the anchored actor through core cascades
  the agreement away.
  ...
  Background:
    ...
    And a VAM agreement titled "Web 3 Consulting HUB" exists for the actor "Web3 ME" sponsored by "industrial_platform"
  ...
  Scenario: Only one active agreement per actor
    Given the VAM agreement "Web 3 Consulting HUB" is activated
    And a VAM agreement titled "Parallel Cycle" exists for the actor "Web3 ME" sponsored by "industrial_platform"
  ...
  Scenario: Terminating citing another agreement's rule fails
    Given the canvas of the VAM agreement "Web 3 Consulting HUB" is replaced with the sample canvas
    And the VAM agreement "Web 3 Consulting HUB" is activated
    And a VAM agreement titled "Other Agreement" exists for the actor "Web3 ME" sponsored by "industrial_platform"
  ...
  Scenario: Platform deletion is blocked while sponsoring agreements
    ...
    Then the response status should be 409
    When I delete the actor "Web3 ME" through the core API
