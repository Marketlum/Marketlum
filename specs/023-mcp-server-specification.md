...
v1 exposes **13 read tools and no writes** (Q3, Q7), tools only — no MCP resources or prompts (Q6).
...
13 read-only tools, `verb_noun` snake_case. Every tool wraps an existing core service call — no new query logic. Each description states what the tool returns **and when to use it** (agent-facing prose, written per tool during implementation).
...
| 13 | `list_recurring_flows` | `RecurringFlowsService` list *(rounds out the roster to 13 reads after dropping the 3 writes — see brainstorm Q7 override)* | `value-streams:read` (same resource as its REST route) | list filters + pagination |
...
| `tool-listing.feature` | 3 | Admin sees all 13; scoped role (e.g. only `invoices:read` + `search:read`) sees exactly its subset; role with no read grants sees an empty list |
| `tool-calls.feature` | 13 | one happy-path call per tool against seeded/fixture data, asserting REST-identical payloads incl. decimal formatting |
...
**Total: 26 scenarios.** Per the strict-BDD rule, feature files and failing step definitions come first; implementation follows.

## Documentation (Q23)

New `## MCP Server` section in the root `README.md`:

- What it is, endpoint URL, auth setup (role → API key), permission model, the 13-tool catalog table.
...
2. BDD feature files (`packages/bdd/features/mcp/`, 26 scenarios) + step definitions in `apps/api/test/mcp/` (failing).
3. Core MCP module: guard, registry, controller + stateless transport wiring, error mapping.
4. The 13 tool definitions.
